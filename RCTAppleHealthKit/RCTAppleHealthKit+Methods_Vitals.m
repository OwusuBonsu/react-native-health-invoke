...
- (void)vitals_getHeadphoneAudioExposureEvent:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback
{
    ...
    HKCategoryType *type = [HKCategoryType categoryTypeForIdentifier: HKCategoryTypeIdentifierHeadphoneAudioExposureEvent];
    ...
    HKSampleQuery *query = [[HKSampleQuery alloc]
        ...
        resultsHandler:^(HKSampleQuery *query, NSArray *results, NSError *error) {
            ...
            NSLog(@"sample for HeadphoneAudioExposureEvent %@", sample);
...
- (void)vitals_getEnvironmentalAudioExposureEvent:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback
{
    ...
    HKCategoryType *type = [HKCategoryType categoryTypeForIdentifier: HKCategoryTypeIdentifierEnvironmentalAudioExposureEvent];
    NSPredicate *predicate = [RCTAppleHealthKit predicateForSamplesBetweenDates:startDate endDate:endDate];
    ...
    HKSampleQuery *query = [[HKSampleQuery alloc]
        ...
        resultsHandler:^(HKSampleQuery *query, NSArray *results, NSError *error) {
            ...
            for (HKQuantitySample *sample in results) {
            NSLog(@"sample for EnvironmentalAudioExposureEvent %@", sample);
            NSString *startDateString = [RCTAppleHealthKit buildISO8601StringFromDate:sample.startDate];
            NSString *endDateString = [RCTAppleHealthKit buildISO8601StringFromDate:sample.endDate];
...
{
    HKQuantityType *appleWalkingSteadiness = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierAppleWalkingSteadiness];
    ...
    [self fetchQuantitySamplesOfType:appleWalkingSteadiness
                                unit:unit
                           predicate:predicate
                           ascending:ascending
                               limit:limit
                          completion:^(NSArray *results, NSError *error) {
                              if(results){
                                  ...
                                  NSLog(@"error getting appleWalkingSteadiness: %@", error);
                                  callback(@[RCTMakeError(@"error getting appleWalkingSteadiness:", error, nil)]);
                                  return;
                              }
                          }];
}
...
- (void)vitals_getLowCardioFitnessEvent:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback
{
    ...
    HKCategoryType *type = [HKCategoryType categoryTypeForIdentifier: HKCategoryTypeIdentifierLowCardioFitnessEvent];
    NSPredicate *predicate = [RCTAppleHealthKit predicateForSamplesBetweenDates:startDate endDate:endDate];
    ...
    HKSampleQuery *query = [[HKSampleQuery alloc]
        ...
        resultsHandler:^(HKSampleQuery *query, NSArray *results, NSError *error) {
            ...
            for (HKQuantitySample *sample in results) {
            NSLog(@"sample for LowCardioFitnessEvent %@", sample);
            NSString *startDateString = [RCTAppleHealthKit buildISO8601StringFromDate:sample.startDate];
            NSString *endDateString = [RCTAppleHealthKit buildISO8601StringFromDate:sample.endDate];
...
- (void)vitals_getIrregularHeartRateEvent:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback
{
    ...
    HKSampleQuery *query = [[HKSampleQuery alloc]
        ...
        resultsHandler:^(HKSampleQuery *query, NSArray *results, NSError *error) {
            ...
            for (HKQuantitySample *sample in results) {
            NSLog(@"sample for IrregularHeartRateEvent %@", sample);
            NSString *startDateString = [RCTAppleHealthKit buildISO8601StringFromDate:sample.startDate];
            NSString *endDateString = [RCTAppleHealthKit buildISO8601StringFromDate:sample.endDate];
...
- (void)vitals_getAppleWalkingSteadinessEvent:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback
{
    ...
    = [RCTAppleHealthKit dateFromOptions:input key:@"startDate" withDefault:nil];
    NSDate *endDate = [RCTAppleHealthKit dateFromOptions:input key:@"endDate" withDefault:[NSDate date]];
    double limit = [RCTAppleHealthKit doubleFromOptions:input key:@"limit" withDefault:HKObjectQueryNoLimit];
...
- (void)vitals_getSixMinuteWalkTestDistance:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback {
    ...
    HKQuantityType *type = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierSixMinuteWalkTestDistance];
...
- (void)vitals_getWalkingSpeed:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback {
    ...
    HKQuantityType *type = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierWalkingSpeed];
    HKUnit *meter = [HKUnit meterUnit];
    HKUnit *second = [HKUnit secondUnit];

    HKUnit *unit = [RCTAppleHealthKit hkUnitFromOptions:input key:@"unit" withDefault:[meter unitDividedByUnit:second]];
...
- (void)vitals_getWalkingStepLength:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback {
    ...
    HKQuantityType *type = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierWalkingStepLength];
...
- (void)vitals_getWalkingAsymmetryPercentage:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback {
    ...
    HKQuantityType *type = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierWalkingAsymmetryPercentage];
    HKUnit *unit = [HKUnit percentUnit];
...
- (void)vitals_getStairAscentSpeed:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback {
    ...
    HKQuantityType *type = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierStairAscentSpeed];
    HKUnit *meter = [HKUnit meterUnit];
    HKUnit *second = [HKUnit secondUnit];
...
- (void)vitals_getStairDescentSpeed:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback {
    ...
    HKQuantityType *type = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierStairDescentSpeed];
    HKUnit *meter = [HKUnit meterUnit];
    HKUnit *second = [HKUnit secondUnit];
